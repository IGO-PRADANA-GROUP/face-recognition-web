...
$jam_masuk = baca_database("","jam_masuk","select * from data_jadwal where hari='$hari'");;
$jam_mulai = menjumlahkan_waktu($jam_masuk,'+0 day +0 hour -15 minutes +0 seconds');
$jam_limit = menjumlahkan_waktu($jam_masuk,'+0 day +0 hour +15 minutes +0 seconds');
...
if (!(empty($id_mahasiswa)))
	
	{
		...
		$tanggal = date('Y-m-d');
		echo $ada =  cek_database("","","","select * from data_absensi where id_mahasiswa='$id_mahasiswa'  and jenis_absensi='masuk' and tanggal='$tanggal'");
		if ($ada == "ada")
		{
		}
		else
